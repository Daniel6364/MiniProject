...
<div style="width:98%; margin-left:10px;">
		<form name="detailForm" action="/listProduct.do?menu=${ requestScope.menu }<%--= menu --%>" method="post">
	...
	<tr>
		<td align="right">
			<select name="searchCondition" class="ct_input_g" style="width:80px">
				...
				<option value="2" <%= ("2".equals(searchCondition) ? "selected" : "")%>>상품가격</option>
				--%>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	<tr>
		...
		<% if( resultPage.getCurrentPage() <= resultPage.getPageUnit() ){ %>
				◀ [prev]
		<% }else{ %>
				<a href="javascript:fncGetProductList('<%=resultPage.getCurrentPage()-1%>')">◀ [prev]</a>
		<% } %>
		...
		<% if( resultPage.getEndUnitPage() >= resultPage.getMaxPage() ){ %>
					[next] ▶
		<% }else{ %>
				<a href="javascript:fncGetProductList('<%=resultPage.getEndUnitPage()+1%>')">[next]▶</a>
		<% } %>
